<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c" %>
<html>
<head>
	<title>User blog  Management Application</title>
</head>
<body>
	<center>
		<h1>User blog Management</h1>
        <h2>
        	<a href="new">Add New User Blog Post</a>
        	&nbsp;&nbsp;&nbsp;
        	<a href="list">List All Blog Posts</a>

        </h2>
	</center>
    <div align="center">
        <table border="1" cellpadding="5">
            <caption><h2>List of Users blog</h2></caption>
            <tr>
                <th>SINO</th>
                <th>TITLE</th>
                <th>DESCRIPTION</th>
                <th>CONTENT</th>
                <th>Actions</th>
            </tr>
            <c:forEach var="user" items="${listUser}">
                <tr>
                    <td><c:out value="${user.sino}" /></td>
                    <td><c:out value="${user.title}" /></td>
                    <td><c:out value="${user.description}" /></td>
                    <td><c:out value="${user.content}" /></td>
                    <td>
                    	<a href="edit?id=<c:out value='${user.sino}' />">Edit</a>
                    	&nbsp;&nbsp;&nbsp;&nbsp;
                    	<a href="delete?id=<c:out value='${user.sino}' />">Delete</a>  
                    </td>
                </tr>
            </c:forEach>
        </table>
    </div>	
</body>
</html>